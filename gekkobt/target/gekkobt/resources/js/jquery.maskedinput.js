...
(function($) {
	...
	var pasteEventName = getPasteEvent() + ".mask", ua = navigator.userAgent, iPhone = /iphone/i
			.test(ua), android = /android/i.test(ua), caretTimeoutId;
	...
	$.fn
			.extend({
				...
				mask : function(mask, settings) {
					...
					settings = $.extend({
						placeholder : $.mask.placeholder, // Load default
															// placeholder
						completed : null
					}, settings);
							...
							.each(
									function() {
										...
										function keydownEvent(e) {
											...
											// backspace, delete, and escape get
											// special treatment
											if (k === 8 || k === 46
													|| (iPhone && k === 127)) {
												pos = input.caret();
										...
										checkVal(); // Perform initial check for
													// existing values
									});
				}
			});
